import { SiteFeature } from "../config/siteConfig";
import { Reveal } from "./Reveal";

export function FeaturesSection({ features }: { features: SiteFeature[] }) {
  return (
    <section className="space-y-5 py-2">
      <Reveal className="max-w-2xl space-y-3">
        ...
        <h2 className="text-3xl font-semibold tracking-tight text-slate-950 md:text-4xl">
          Update the story for this business without changing the shared app shell.
        </h2>
      </Reveal>
      <div className="grid gap-5 md:grid-cols-3">
        {features.map((feature, index) => (
          <Reveal key={feature.id} delayMs={index * 100}>
